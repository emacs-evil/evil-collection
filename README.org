...
~evil-collection-theme-defaults~ for the existing list.

For example, ~vterm~, ~eat~ and ~ghostel~ all need a way to send toggle sending
ESC to Emacs or the underlying binary.

To customize, override with ~completion-category-overrides~.

Each entry has the form ~(ID . PLIST)~ with these properties:

| Property   | Accepts                            | Notes                                                     |
|------------+------------------------------------+-----------------------------------------------------------|
| ~:enabled~ | ~t~, ~nil~, or a function          | Missing is treated as ~t~. Functions are called with no args. |
| ~:state~   | symbol or list of symbols          | Evil states. Singular form (e.g. ~normal~) is allowed.    |
| ~:key~     | key string or list of key strings  | Strings suitable for ~kbd~. Singular form is allowed.     |
...
#+end_src
 ...
 ~vterm~, ~eat~, and ~evil-ghostel~ will then bind their respective
"toggle ESC routing" command to ~normal~ and ~insert~ state bounded to the key
C-c C-z.
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((find-usages :key "gR")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((find-definition :key "gD")
        (pop-definition  :key "C-o")))     ;; or :enabled nil to disable either
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((lookup-doc :key "gh")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((goto-repl :key "gZ")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((find-file :key "gF")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((quit :key "Q")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((describe-mode :key "gH")))     ;; or :enabled nil to disable
#+end_src
...
(setq evil-collection-theme-overrides
      '((refresh     :key "gu")
        (refresh-all :key "gU")))     ;; or :enabled nil to disable either
#+end_src
...
(setq evil-collection-theme-overrides
      '((next-item      :key "gj")
        (prev-item      :key "gk")
        (next-section   :key ("]]" "C-j"))
        (prev-section   :key ("[[" "C-k"))
        (next-section-2 :key "C-j")
        (prev-section-2 :key "C-k")))     ;; or :enabled nil to disable
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq evil-collection-theme-overrides
      '((debug-continue   :key "c")            ;; default also binds <f5>
        (debug-step-over  :key "n")            ;; default also binds <f10>
        (debug-step-into  :key ("i" "s"))      ;; default also binds <f11>
        (debug-step-out   :key "o")            ;; default also binds <S-f11>
        (debug-breakpoint :key "b")))          ;; default also binds <f9>
#+end_src

Disable any with =:enabled nil=.
